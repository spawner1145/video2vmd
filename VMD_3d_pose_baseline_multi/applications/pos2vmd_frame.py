...
# Set by the pipeline for the optional camera-facing head experiment.
FORCE_HEAD_FACING_CAMERA = False
...
def position_to_frame_head(frame, pos, pos_gan, upper_body_rotation1, upper_body_rotation2, upper_correctqq, is_gan, slope_motion):
    if FORCE_HEAD_FACING_CAMERA:
        # Cancel the parent upper-body orientation in the neck and keep the
        # head local rotation neutral, yielding a world-camera-facing head.
        return (upper_body_rotation2.inverted() * upper_body_rotation1.inverted(),
                QQuaternion())
    if is_gan: 
