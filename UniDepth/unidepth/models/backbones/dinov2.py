...
def _make_dinov2_model(
    ...
    **kwargs,
    ...
    if pretrained == "":
        ...
        logger.debug("Loaded remote DINOv2 weights: %s", info)
    elif pretrained is not None:
        state_dict = torch.load(pretrained, map_location="cpu")
        info = model.load_state_dict(state_dict, strict=False)
        logger.debug("Loaded DINOv2 weights from %s: %s", pretrained, info)
    else:
        logger.debug("Backbone weights will be loaded by the parent UniDepth checkpoint")
    return model
